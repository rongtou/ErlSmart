...
class SmartGoto(object):
    ...
    def run(self, kind, point):
        ...
        if option_len == 1:
            ...
            self.window.show_quick_panel(list(map(lambda o: "{}:{}:{}:{}".format(o[0], o[1], o[2], o[3]), self.options)), self.on_done)
        else:
    ...
    def on_done(self, index):
        if index >= 0:
            self.window.open_file('{0}:{1}'.format(self.options[index][4], self.options[index][3]), sublime.ENCODED_POSITION)
